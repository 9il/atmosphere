	...
	function traverser(node, $parentList) {
		for(var name in node.members) {
			...
			if(member.type == 'package') {
				...
				if(member.qualifiedName == Title || member.qualifiedName == Title + PackageSeparator + 'package') { // Current module.
					$elem.find('a').append(' <i class="icon-asterisk"></i>');
					updateBreadcrumb(member.qualifiedName, SourceRepository);
					var $up = $parentList;
